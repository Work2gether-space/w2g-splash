...
// Change: Default OMADA_BASE now points to the Cloudflared tunnel host.
...
async function fWithCookies(url, opts = {}, jar = [], refererPath = "/portal") {
  ...
  if (!headers.has("User-Agent")) headers.set("User-Agent", "w2g-splash/2025-09-04");
...
// Optional helper so callers can confirm the effective base in diagnostics
function getOmadaBase() {
  return OMADA_BASE;
}

module.exports = { hotspotLogin, macColons, macHyphens, macRaw, getOmadaBase };
